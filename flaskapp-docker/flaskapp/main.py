...
def loadFile():
	...
	if extension == 'pdf':
		try:
			...
			fichierJson = json.dumps({'Metadonnees':{'Nom':name,'mimetype':mimetypeFile(),'Infos':document.getDocumentInfo()},'Donnees':pdftext})
	...
	#Images
		...
		try:
			...
			if extension in {'png','jpg','jpeg'}:
				...
				imageJson = json.dumps({'Metadonnees':{'Nom':name, 'extension':extension,'mimetype':mimetypeFile(),'exif':img._getexif(),'rekognition': meta}, 'Donnees':imageB64.decode("utf-8")})
				os.remove(os.path.join(app.config['UPLOAD_FOLDER'], filename))
			else: 
				imageJson = json.dumps({'Metadonnees':{'Nom':name, 'extension':extension,'mimetype':mimetypeFile() },'Donnees':imageB64.decode("utf-8")})
			with open(os.path.join(app.config['UPLOAD_FOLDER'],name +'.json'),"w") as file: 
	...
	# Fichiers CSV
		...
		try:
			...
			CSVJson = json.dumps({'Metadonnees':{'Nom':name, 'extension':extension,'mimetype':mimetypeFile(),'Nombre de lignes':rowNumber,'Nombre de colonnes':columnNumber},'Donnees':liste})
			with open(os.path.join(app.config['UPLOAD_FOLDER'],name +'.json'),"w") as file: 
	...
	# Fichiers txt
		...
		try:
			texte = str(file.read())
			txtJson = json.dumps({'Metadonnes':{'Nom':name, 'extension':extension,'mimetype':mimetypeFile(), 'Nombre de caractères':len(texte)}, 'Donnees':texte})
			with open(os.path.join(app.config['UPLOAD_FOLDER'],name +'.json'),"w") as file: 
